...
*   **Graph Store (PostgreSQL):** The source of truth for structured `MemoryObject`s and their relationships, enabling structured queries and graph traversal.
...
1.  **Hybrid Extraction:** A fast **regex extractor** handles simple patterns before a **spaCy NER model** tackles complex entities.
2.  **Classification:** Raw text is classified into a `memory_type` and checked for negation.
3.  **Batch Embedding:** Memory content is vectorized in batches for high efficiency.
4.  **Caching:** Both NER and embedding generation are cached (`LRU Cache`) to eliminate redundant processing.
5.  **Dual Storage:** Data is stored in PostgreSQL, and embeddings are upserted to Qdrant in a batch operation.

#### **3.2. The Hybrid Retrieval Algorithm (Enhanced)**

The retrieval process is designed for high accuracy and low latency.

1.  **Parallel Search:** The query is simultaneously used for a **semantic search** in Qdrant and a **keyword search** in PostgreSQL.
2.  **Candidate Merging & Hydration:** Unique results from both searches are collected, and their full data is fetched from PostgreSQL.
3.  **Hybrid Re-Ranking:** Candidates are re-scored using a weighted formula that combines vector similarity, keyword similarity, graph centrality (placeholder), importance, and recency decay.
4.  **Final Ranking:** The top 10 results are returned.
