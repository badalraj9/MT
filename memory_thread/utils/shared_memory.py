...
class AlignedSharedMemory:
    ...
    def __init__(self, size: int, alignment: int = 64, create=True, name=None):
        ...
        # Find the first memory address that is a multiple of the alignment
...
class SlabAllocator:
    def __init__(self, num_slabs: int, slab_size: int, shm_class=AlignedSharedMemory):
        self.num_slabs = num_slabs
        self.slab_size = slab_size

        # Data Slabs
        self.data_shm = shm_class(size=num_slabs * slab_size)

        # Metadata
        self.metadata_shm = shm_class(size=num_slabs * ctypes.sizeof(ctypes.c_int))
        ...
        with self.lock:
            for i in range(self.num_slabs):
                ...
                if self.metadata[slab_id] == FREE:
                    ...
                    log.info(f"Reserved slab {slab_id}")
        ...
        with self.lock:
            for slab_id in range(self.num_slabs):
                if self.metadata[slab_id] == WRITTEN:
                    ...
                    log.info(f"Worker got slab {slab_id}")
                    offset = slab_id * self.slab_size
                    slab_memory = self.data_shm.memory[offset:offset + self.slab_size]
                    return SlabHandle(slab_id, slab_memory)
        return None
