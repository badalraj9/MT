...
def classify_memory(text: str) -> Tuple[str, bool]:
    """
    Returns memory_type and negation_flag based on linguistic rules.
    """
    ...
    # Identity (dominates)
    if has_identity:
        return "identity", has_negation

    # Preference
    if has_preference:
        return "preference", has_negation

    # Temporal event
    if has_temporal:
        return "event", has_negation

    # Negation without stable → event
    if has_negation:
        return "event", has_negation
