...
def ingest_text(text: str, importance: float = 0.5):
    ...
    memory = MemoryObject(
        id=memory_id,
        content=text,
        # The classified_type from the service is now used directly.
        # Note: 'timeline_update' is a signal, not a storable type. The actual
        # memory_type will be handled by the timeline_service. For now, we
        # default back to 'event' if we get this signal.
        memory_type=classified_type if classified_type != "timeline_update" else "event",
