...
def ingest_text(text: str, importance: float = 0.5):
    ...
    # The new MemoryObject uses UUID and default factories
    memory = MemoryObject(
        content=text,
        ...
        last_accessed=now,
        embedding=embedding
        # Domain, history, etc., will be handled by the timeline_service
    )

    # Step 7: Store in Postgres and Qdrant
    ...
    log.info(f"Placeholder: Running async eviction check for memory ID {memory.id}.")
